...
for ($month = 1; $month <= 12; $month++) { //Bucle para recorrer los 12 meses
    $time = mktime(0, 0, 0, $month, 1, $year); //Obtener la fecha del primer día del mes
    $date = getdate($time);
    $dayTotal = cal_days_in_month(0, $date['mon'], $date['year']); //Obtener el número de días del mes
    //Print the calendar header with the month name.
    print '<table><tr><td colspan="7"><strong>' . $date['month'] . '</strong></td></tr>';
    for ($i = 0; $i < 6; $i++) {
        print '<tr>';
